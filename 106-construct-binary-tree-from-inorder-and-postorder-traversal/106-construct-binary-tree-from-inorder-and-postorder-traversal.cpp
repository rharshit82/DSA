...
public:
    TreeNode* buildTree(vector<int>& inorder, vector<int>& postorder) {
        ...
        TreeNode* res = construct(inorder, 0, inorder.size()-1,postorder, 0, postorder.size()-1, inMap);
        return res;
    }
    TreeNode* construct(vector<int>& inorder,int inStart,int inEnd,vector<int>& postorder, int postStart, int postEnd, map<int,int>&inMap){
        ...
        root->left = construct(inorder, inStart, inStart+ numsLeft, postorder, postStart, postStart+numsLeft-1, inMap);
